...
export default function AuditLog() {
  ...
  if (!audit) return <div style={{ padding: "20px", color: "var(--text-muted)" }}>Loading...</div>;
  ...
  return (
    <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: "6px", overflow: "hidden" }}>
      ...
      {audit.length === 0 ? (
        <div style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)", fontSize: "10px" }}>No audit events — system just started</div>
      ) : (
        <div style={{ maxHeight: "500px", overflow: "auto" }}>
          {audit.map((e: any, i: number) => {
            const evt = e.event_type || e.type || "UNKNOWN";
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "4px 12px", fontSize: "10px", borderBottom: "1px solid var(--border-subtle)" }}>
                <span style={{ color: "var(--text-muted)", fontVariantNumeric: "tabular-nums", width: "55px" }}>{formatTimestamp(e.timestamp)}</span>
                <span style={{
